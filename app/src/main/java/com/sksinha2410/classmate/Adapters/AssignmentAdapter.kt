...
class AssignmentAdapter(options: FirebaseRecyclerOptions<Books?>, val clubName: String) :
    FirebaseRecyclerAdapter<Books?, AssignmentAdapter.userAdapterHolder?>(options) {
        ...
        try {
            ...
            holder.bookCard.setOnClickListener {
                try {
                    ...
                    Toast.makeText(holder.name.context, "Assignment Link not found", Toast.LENGTH_SHORT)
                        .show()
                }
            }
        } catch (e: Exception) {
