...
class BookAdapter(options: FirebaseRecyclerOptions<Books?>) :
    FirebaseRecyclerAdapter<Books?, BookAdapter.userAdapterHolder?>(options) {
        ...
        try {
            ...
            holder.bookCard.setOnClickListener {
                try {
                    ...
                    Toast.makeText(holder.name.context, "Book Link not found", Toast.LENGTH_SHORT)
                        .show()
                }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BookAdapter.userAdapterHolder {
